...
### 📝 Critérios de Aceitação das Histórias Principais (Topo do Backlog)

#### **US01 — Dashboard de Chamados por Status (P1)**
- **Critério 1:** O painel deve exibir cartões numéricos com a contagem exata de chamados nos status `PENDENTE`, `EM_ESPERA` e `CONCLUIDO`.
- **Critério 2:** Ao alterar o status de qualquer solicitação no sistema, os contadores do dashboard devem refletir a nova contagem sem necessidade de reinicialização da aplicação.
- **Critério 3:** Caso não haja chamados cadastrados, o painel deve exibir valor zero para todos os status de forma legível.

#### **US02 — Abertura de Solicitação de Manutenção (P1)**
- **Critério 1:** O formulário deve exigir obrigatoriamente: seleção de Sala, seleção de Equipamento (pertencente à sala selecionada), Descrição do defeito e Prioridade inicial sugerida.
- **Critério 2:** O sistema não deve permitir submissão com campos vazios ou descrição com menos de 10 caracteres.
- **Critério 3:** Ao submeter com sucesso, a solicitação deve ser criada com status `PENDENTE`, exibindo mensagem de confirmação para o usuário.

#### **US03 — Alteração de Status e Prioridade da Solicitação (P1)**
- **Critério 1:** O técnico deve ter acesso a uma interface de edição rápida do chamado, podendo mudar o status para `PENDENTE`, `EM_ESPERA` ou `CONCLUIDO`.
- **Critério 2:** O técnico pode alterar a prioridade entre `BAIXA`, `MEDIA` ou `ALTA`.
- **Critério 3:** Cada alteração deve atualizar o timestamp da última atualização e persistir no banco de dados.
...
* **Vídeo de Apresentação (Sprint 0 - ~5 min):** [Link do Vídeo no YouTube / Drive](https://youtube.com) *(A ser gravado pela equipe)*